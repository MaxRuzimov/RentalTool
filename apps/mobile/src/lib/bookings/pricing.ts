 ...
 * docs/design/m5-booking-spec.md §3.3/§11). Ported CHARACTER-FOR-CHARACTER
 * identical from apps/web/src/lib/bookings/pricing.ts per
 * docs/design/m8-mobile-spec.md §5.3.2, so web and mobile always compute the
 * same number for the same inputs — do not "clean up" the formulas here
 * without also updating the web copy. Used by both the live request-to-rent
 * estimate and booking list rows (mine / owner-requests) so the two never
 * drift apart — see the M5 task's explicit "don't introduce an off-by-one"
 * warning.
 */
